...
function encodeAvatarPixels(pixels: Array<string | null>): string {
  const normalized = new Array<string | null>(AVATAR_TOKEN_COUNT).fill(null);
...
export function normalizeAvatarCode(code: string | null | undefined, fallback: string = DEFAULT_AVATAR_CODE): string {
  const clean = cleanAvatarCode(code);
  return isAvatarCode(clean) ? clean : fallback;
}
